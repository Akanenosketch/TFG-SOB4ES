...
Este anexo funciona como *manual de usuario* para reproducir de principio a fin el _pipeline_ descrito en la #link(<arquitectura>)[*Arquitectura general*]: desde la configuración del entorno y las credenciales de los servicios externos, hasta la ejecución de los _notebooks_ de ingesta, procesamiento, modelado y evaluación.
...
- *Jupyter* y *nbconvert*, necesarios para ejecutar y automatizar los _notebooks_ (véase #link(<elementos-auxiliares>)[*Anexo IV*]).
...
La forma recomendada de autenticarse es la misma que se usa dentro de los propios _notebooks_ de ingesta, ejecutando estas dos líneas en una celda de Python:
...
*`ee.Authenticate()`* guarda un _token_ de credenciales localmente (en `~/.config/earthengine/credentials` en Linux/macOS), de forma que en próximas ejecuciones, incluidas las de los propios _notebooks_ del _pipeline_, basta con llamar a *`ee.Initialize(project=...)`*, sin repetir *`ee.Authenticate()`*.
...
#rect[
  *Problema conocido: bloqueo de la autenticación de GEE* \ \
  Cuando el _pipeline_ se lanza, la llamada a `ee.Authenticate()` abre un navegador y levanta un servidor local para recibir la respuesta de OAuth. 
  
  Una vez autenticado correctamente, el proceso es bloqueado directamente por los propios sistemas de seguridad de Google, que interpretan la petición automática como sospechosa (en la práctica, la infraestructura de Google acaba bloqueando la propia autenticación contra Google). \ \
  *Solución:* autenticar la cuenta manualmente *una sola vez*, antes de lanzar cualquier ejecución automatizada, usando el modo de autenticación por CLI orientado a _notebooks_:

  ```bash
  earthengine authenticate --auth_mode=notebook
  ```

  Este modo genera una URL que se abre manualmente en cualquier navegador, tras lo cual se pega el código de verificación en la terminal. El _token_ resultante se guarda en `~/.config/earthengine/credentials`, igual que con `ee.Authenticate()`, por lo que las ejecuciones posteriores solo necesitan llamar a `ee.Initialize(project=...)` y no vuelven a disparar el flujo de navegador que provoca el bloqueo.
]

Como alternativa por terminal (equivalente, pero fuera del _notebook_) existe también el comando `earthengine authenticate` sin argumentos, aunque para este _pipeline_ se recomienda el flujo anterior (`--auth_mode=notebook`) precisamente para evitar el problema descrito arriba, especialmente si se van a usar los _scripts_ de automatización del #link(<elementos-auxiliares>)[*Anexo IV*].
...
La librería `cdsapi` lee este fichero automáticamente al ejecutar cualquier solicitud de descarga, independientemente de que se ejecute desde un _notebook_ o desde un _script_.

*Documentación oficial:* https://cds.climate.copernicus.eu/how-to-api

==== Copernicus DEM (vía AWS S3)

*No requiere autenticación ni registro:* el _bucket_ público de AWS S3 que aloja el DEM (GLO-30) es accesible directamente mediante `rasterio` indicando la URL correspondiente, tal como se describe en la #link(<capa-ingesta>)[*Capa de ingesta de datos*].
...
Una vez recibidos los datos del proyecto SOB4ES por parte del tutor (véase el aviso al inicio de este anexo), colócalos en la estructura de carpetas que esperan los _notebooks_ de ingesta:
...
#rect[
  *Nota:* la estructura exacta de carpetas debe verificarse contra el `.gitignore` del repositorio y contra el primer _notebook_ de la #link(<capa-ingesta>)[*Capa de ingesta de datos*], ya que las rutas de lectura pueden haberse fijado como rutas relativas concretas durante el desarrollo.
]
...
Para GEE, la comprobación debe hacerse desde una celda de _notebook_ (o desde una sesión interactiva de Python), no con `python -c` en una sola línea, ya que `ee.Authenticate()` necesita abrir el navegador de forma interactiva la primera vez:
...
Si alguno de los pasos anteriores falla, revisa primero la sección correspondiente de este anexo antes de continuar con la ejecución de los _notebooks_. 
