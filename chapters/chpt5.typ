...
#rect[ “_Machine Learning_ es el campo de estudio que le otorga a las computadoras la capacidad de aprender sin explícitamente programarlo.”]

Otro personaje relevante dentro del campo de la inteligencia artificial es *Tom Mitchell*#sub([@geronHandsOnML, @tomMitchellWikipedia]), creador del ordenador que es capaz de “leer la mente humana” a partir de imágenes del cerebro humano#sub([@reutersMindImages]). Él lo define de una forma más técnica, siendo la siguiente una de las definiciones más citadas dentro de la literatura académica#sub([@mitchellMachineLearning]):

#rect[“Se dice que un programa de ordenador aprende de una experiencia E con respecto a una tarea T y una media de rendimiento P, si su rendimiento en T, medido por P, mejora con la experiencia E”]
...
Algunos modelos representativos de este paradigma son la regresión lineal, los árboles de decisión, los métodos de ensamblado (_Random Forest_#sub([@breimanRandomForests]), XGBoost#sub([@xgboostIBM])) y las redes neuronales entrenadas con ejemplos etiquetados, varios de los cuales se emplean en este propio trabajo (véase #link(<modelos-de-aprendizaje-automatico-empleados>)[*Modelos empleados*]).
...
$ EE[(y-hat(f)(x))^2] = (EE[hat(f)(x)] - f(x))^2 + "Var"[hat(f)(x)] + sigma^2_epsilon $
...
+ *Modelo por _target_*: Consiste en entrenar $K$ modelos de regresión de salida única, uno por cada $y_k$, ignorando cualquier relación entre los diferentes _targets_.
+ *Multisalida simétrica o nativa*: Consiste en entrenar un único modelo que predice el vector completo $y$ a la vez, compartiendo parámetros o estructura interna entre todos los _targets_.
+ *Multisalida encadenada o direccional*: Consiste en entrenar una secuencia de modelos donde cada uno usa las predicciones de los modelos anteriores como variables predictoras o _features_ adicionales junto a las ya existentes.

Desde la perspectiva del compromiso sesgo-varianza, comentado en el apartado anterior, compartir parámetros entre los _targets_ (estrategias 2 y 3) puede ser visto como una forma adicional de regularización: si los $K$ _targets_ están correlacionados entre sí, forzar al modelo a explicarlos con una representación compartida reduce la varianza efectiva del conjunto de modelos, a costa de introducir un sesgo si algún _target_ concreto se comporta de forma distinta al resto.

Estas tres estrategias, junto con la comparación empírica entre ellas, son precisamente el objeto de estudio de la #link(<comparativa-modelos>)[*Comparación de los modelos empleados*] y los resultados que se obtuvieron en las diferentes pruebas adicionales, las cuales se pueden ver en el #link(<pruebas-llevadas-a-cabo>)[*Anexo V*].
...
+ *_Clustering_ o agrupamiento*: Agrupan las observaciones que se determinan como similares entre sí, aunque no exista una categoría predefinida. Esto es común en algoritmos como _k-means_ o el _clustering_ jerárquico.
+ *Reducción de la dimensionalidad*: Buscan representar los datos originales con el menor número de variables posibles, conservando la mayor parte posible de la información o varianza original. Esto es común en el Análisis de Componentes Principales o PCA.
+ *Estimación de densidad y detección de anomalías*: Modelan la distribución de probabilidad $p(x)$ subyacente a los datos, de forma que las muestras con baja probabilidad pueden ser señaladas por el modelo aprendido como atípicas o anómalas.
+ *Modelado generativo*: Aprenden a generar nuevas muestras sintéticas que sigan la misma distribución que los datos de entrenamiento. Son elemento base, entre otras cosas, de los modelos de fundación, los cuales se mencionan en el apartado siguiente. Común en los _autoencoders_ y en los modelos de difusión.

El aprendizaje no supervisado no se ha empleado en este trabajo como técnica de modelado principal, ya que se dispone en todo momento de mediciones reales de biodiversidad que actúan como salida conocida; no obstante, algunas de sus ideas, como la *reducción de variables correlacionadas (reducción de la dimensionalidad)*, están relacionadas con la selección de variables descritas en la #link(<seleccion-de-variables>)[*Selección de variables*].
...
*_Clustering_ (_K-Means_)*: dado un número de grupos $k$, se buscan centroides $mu_1, ..., mu_k$ que minimicen la suma de distancias al cuadrado dentro de cada grupo (inercia):
...
- *_Co-training_*: Dos modelos entrenados sobre vistas distintas de los datos etiquetan mutuamente los ejemplos sin etiquetar del otro modelo.
...
Formalmente se suele modelar como un Proceso de Decisión de Markov (MDP), y entre sus algoritmos más conocidos están Q-learning y sus variantes basadas en redes neuronales profundas (_Deep Reinforcement Learning_), popularizadas por sistemas como AlphaGo#sub([@googleDeepMindAlphaGo]).
...
Antes de entrenar los modelos descritos en la sección anterior, es habitual reducir el conjunto de variables predictoras candidatas a un subconjunto más manejable y menos redundante. Esto no solo simplifica el modelo, sino que en modelos como_ Ridge_ resulta prácticamente necesario cuando existe colinealidad severa (véase #link(<regresion-ridge>)[*Regresión_ Ridge_*]). 
...
- *Correlación entre cada variable predictora y el _target_:* permite descartar variables con relación prácticamente nula con lo que se quiere predecir con independencia de su relación con el resto de variables predictoras.
...
- *Regularización L1 (Lasso):* a diferencia de la penalización L2 de_ Ridge_ (véase #link(<regresion-ridge>)[*Regresión_ Ridge_*]), la penalización L1 puede llevar coeficientes exactamente a cero, realizando así una selección de variables implícita durante el propio ajuste del modelo lineal. 
...
Además de la tabla de variables predictoras cuyas definiciones se pueden ver en #link(<informe-eda>)[*Anexo I*], en la #ref(<tab-2>) se muestran las variables seleccionadas como variables objetivo (_targets_) a predecir en los modelos desarrollados.
...
Además, en el #link(<modelos-empleados>)[*Anexo II*], se podrá ver el código base de todos los modelos empleados.

=== Regresión_ Ridge_ <regresion-ridge>

Los modelos de regresión_ ridge_, son una variante de los modelos de regresión base, pero que cuenta con regularización para corregir el sobreajuste de los datos de entrenamiento cuando se están desarrollando modelos de aprendizaje automático o _Machine Learning_.
...
_Ridge_ añade una penalización proporcional al cuadrado de la magnitud de los coeficientes, controlada por el hiperparámetro $alpha$:

$ hat(w)_"ridge" = op("arg min")_w quad parallel y - X w parallel^2 + alpha parallel w parallel^2 $

Cuando $alpha = 0$,_ Ridge_ se reduce a la regresión lineal ordinaria, pero a medida que $alpha$ aumenta, los coeficientes se “encogen” hacia cero sin llegar nunca a anularse por completo en comparación con Lasso, reduciendo la varianza del modelo a costa de introducir un pequeño sesgo.

A diferencia de la regresión lineal ordinaria, que no tiene una solución estable cuando existe colinealidad perfecta o casi perfecta entre variables,_ Ridge_ sí tiene siempre una solución cerrada única, la cual es la siguiente:

$ hat(w) = (X^T X + alpha I)^(-1) X^T y $

#rect[*Nota:* La colinealidad entre variables en_ Ridge_ provoca que la matriz $X^T X$ deje de ser invertible o que quede mal condicionada.]

Esto permite que al sumar $alpha I$ a $X^T X$ se garantice que la matriz sea invertible independientemente del grado de colinealidad entre las diferentes variables predictoras, propiedad relevante para este TFG, donde varias de las variables edáficas y climáticas están correlacionadas entre sí debido a que proceden de fuentes relacionadas (véase #link(<informe-eda>)[*Anexo I*]).

#colbreak()

*Grados de libertad efectivos y elección de $alpha$:* Una forma útil de entender el efecto de la regularización es a través de los llamados grados de libertad efectivos del modelo:

$ "df"(alpha) = sum_(j=1)^p d_j^2 / (d_j^2 + alpha) $

donde *$d_j$* son los valores singulares de $X$.

Cuando $alpha = 0$, df($alpha$) = p. Esto implica que se usan todos los grados de libertad del modelo lineal. A medida que crece $alpha$, df($alpha$) disminuye, reflejando que el modelo efectivamente emplea menos parámetros libres.

*Requisito de estandarización:*

La interpretación de los coeficientes de_ Ridge_ en términos de importancia relativa sólo es válida si las variables predictoras están en la misma escala, ya que la penalización $parallel w parallel^2$ trata todos los coeficientes por igual independientemente de la escala original de la variable a la que multiplican.
...
=== _Random Forest_ <random-forest>

_Random Forest_, propuesto por Breiman en 2001#sub([@breimanRandomForests]), es un método de ensamblado de modelos basado en árboles de decisión que combina las predicciones de múltiples árboles entrenados de forma independiente para obtener una predicción más robusta que la de un único árbol. Se apoya en dos fuentes de aleatoriedad, de las que toma su nombre de _Random Forest_:
...
Esta expresión es clave para entender por qué _Random Forest_ introduce la selección aleatoria de variables además del *_bagging_*: el _bagging_ por sí solo ya reduce el segundo término (el que decrece con $N$), pero los árboles resultantes tienden a estar muy correlacionados entre sí (comparten las variables más predictivas en los primeros _splits_); al forzar que cada división considere sólo un subconjunto aleatorio de variables, se reduce $rho$, y por tanto el primer término, que es el que no desaparece por mucho que se aumente $N$.

*Importancia de variables:* _Random Forest_, en su implementación de scikit-learn#sub([@scikitLearn]) proporciona de forma nativa dos formas de estimar la importancia de cada variable predictora: la *importancia por impureza media (MDI)*, calculada como la reducción media de varianza (en regresión) que aporta cada variable en los nodos donde se ha usado para dividir, ponderada por la proporción de muestras que llegan a cada nodo; y la *importancia por permutación (_permutation importance_)*, que mide la caída de rendimiento del modelo al barajar aleatoriamente los valores de una variable en el conjunto de validación, manteniendo el resto intactas. La segunda es más robusta frente a variables con muchos niveles o alta cardinalidad, aunque computacionalmente más costosa.

Ambas medidas se han empleado en este TFG como complemento a SHAP#sub([@shapDocs]) (véase #link(<capa-evaluacion>)[*Capa de evaluación de modelos*]).

Se ha considerado este modelo, junto con su variante multisalida, por su buen comportamiento con conjuntos de datos de tamaño reducido como el de este TFG, su tolerancia a variables predictoras correlacionadas entre sí, y por proporcionar de forma nativa una medida de importancia de variables, complementando a SHAP (véase la #link(<capa-evaluacion>)[*Capa de evaluación de modelos*]).

==== _Random Forest_ multisalida <random-forest-multisalida>
...
Esto significa que, a diferencia de entrenar un único modelo independiente por indicador/_target_, la variante multisalida permite que un mismo árbol capture patrones de división que sean simultáneamente informativos para varios _targets_ correlacionados entre sí sin necesidad de un mecanismo explícito de encadenamiento como es RegressorChain, el cual se explica con más detalle en las siguientes secciones.

La predicción para el _target_ k-ésimo en la variante multisalida sigue siendo el promedio de las predicciones de los N árboles del bosque para ese _target_ concreto:

$ hat(y)_k = 1/N sum_(i=1)^N hat(y)_(i,k) $

pero, a diferencia de entrenar $K$ bosques independientes (uno por _target_), aquí los $N$ árboles son compartidos entre todos los _targets_, lo que reduce el coste computacional total y permite comparar directamente si el aprendizaje conjunto aporta ventaja frente a la variante independiente (véase #link(<pruebas-llevadas-a-cabo>)[*Anexo V*]).

#colbreak()

=== XGBoost <xgboost>

En *XGBoost* o también conocido como _*Gradient Boosting*_, crea y construye los árboles de forma secuencial (a diferencia de _Random Forest_ que los entrena y construye de forma independiente y en paralelo) para que cada árbol nuevo sea entrenado para corregir los errores o residuos cometidos por su conjunto de árboles predecesor ya entrenados.
...
Este enfoque secuencial permite habitualmente alcanzar un rendimiento superior al de _Random Forest_ con menos árboles, pero también lo hace más sensible al sobreajuste si el número de árboles o la tasa de aprendizaje no se ajustan correctamente.

*Objetivo regularizado:* A diferencia del _gradient boosting_ clásico, XGBoost optimiza en cada iteración una aproximación de segundo orden (expansión de Taylor) de la función de pérdida, e incluye un término de regularización explícito sobre la complejidad del árbol añadido#sub([@chenXGBoost]):
...
+ *`one_output_per_tree`:* Opción por defecto, se entrena un conjunto de árboles independiente por cada _target_, de forma equivalente a entrenar K modelos XGBoost por separado, aunque compartiendo la misma llamada de entrenamiento.
+ *`multi_output_tree`:* Cada árbol se entrena para predecir todos los _targets_ a la vez de forma similar a cómo lo realiza _Random Forest_ en su variante multisalida de forma nativa, permitiendo que las divisiones del árbol capturen relaciones compartidas entre _targets_ correlacionados.

En este TFG se ha empleado la estrategia *`multi_output_tree`*, precisamente para poder comparar, en igualdad de condiciones con la variante multisalida de _Random Forest_, si el aprendizaje conjunto de los distintos indicadores de biodiversidad aporta una ventaja real frente al modelado independiente, comparación que en las #link(<conclusiones>)[*Conclusiones*] de este TFG resulta favorable a XGBoost multisalida frente al resto de modelos.
...
De esta forma, si los indicadores de biodiversidad están relacionados entre sí, el modelo puede aprovechar esa relación de forma explícita y direccional para mejorar la predicción de los _targets_ posteriores de la cadena.

Dentro de este TFG se hace uso de *_Random Forest_* como modelo base del *RegressorChain* y los hiperparámetros se ajustan una única vez dentro del modelo base, no se calculan por _target_ ni por posición, sino sobre el conjunto de entrenamiento completo.

El orden en el que se recorren los _targets_ y las cadenas anteriores es un detalle que determina la salida final del modelo, es decir, el orden de las cadenas tienen un impacto directo sobre el resultado final. Esto se debe a que un _target_ situado al principio de la cadena nunca recibe como entrada las predicciones de los demás, mientras que uno situado al final se beneficia de todos los anteriores.

Para decidir el orden de las cadenas, existen diferentes formas de hacerlo:

- *Manual:* Consiste en escoger un criterio, como puede ser la varianza de cada _target_, y ordenar a partir de ese criterio. Es la opción más interpretable, ya que el orden tiene un significado defendible de antemano, pero depende de que el motivo y el conocimiento previo sean correctos.
- *Basado en los propios datos:* Consiste en calcular algún estadístico de asociación entre _targets_ y encadenar primero los _targets_ más relacionados entre sí. Dentro de la literatura revisada (véase #link(<referencias>)[*Referencias*]) esta estrategia se considera como una estrategia de segundo orden, por modelar relaciones entre _targets_, frente a las estrategia de primer orden y las de nivel superior, que intentan capturar relaciones conjuntas entre dos o más _targets_ a la vez#sub([@readPfahringerReview2021]).
- *Como problema de búsqueda:* Consiste en hacer uso de técnicas como _beam search_, métodos de Monte Carlo#sub([@readMartino2020]) o algoritmos genéticos#sub([@readPfahringerReview2021]) dado que evaluar todas las posibles combinaciones de un set de _targets_ puede volverse intratable. Estos métodos suelen mejorar el resultado de un orden manual o de uno que sea puramente aleatorio, pero exigen entrenar y evaluar la cadena completa muchas veces antes de llegar al orden final, con el consecuente coste computacional.
- *A partir de un modelo probabilístico de dependencia:* Consiste en hacer uso de algún modelo probabilístico, como por ejemplo, las redes bayesianas#sub([@sucarBayesianChains]) aprendido sobre los propios _targets_. Los _targets_ que no dependan de otros irán de primeros en la cadena y los que dependan de más _targets_ irán más tarde.

Cabe indicar que, ninguna de estas estrategias garantiza encontrar un orden verdaderamente óptimo, y todas comparten el mismo riesgo común: que el orden elegido perjudique de forma considerable a los _targets_ que queden peor posicionados en la cadena.

#colbreak()

=== Redes neuronales <redes-neuronales>

Como alternativa de aprendizaje profundo a los modelos anteriores, se ha entrenado también un *perceptrón multicapa* (_Multi-Layer Perceptron_, MLP), implementado con PyTorch#sub([@pytorchDocs]). Un MLP está formado por una capa de entrada, una o varias capas ocultas y una capa de salida, donde cada capa transforma la salida de la anterior:
...
En esta variante, la capa de salida de la red tiene tantas neuronas como _targets_ a predecir, y la red se entrena minimizando el *error cuadrático medio (MSE) estándar*, calculado y promediado conjuntamente sobre todos los _targets_ a la vez:

$ L(y, hat(y)) = 1/K sum_(k=1)^K (y_k - hat(y)_k)^2 $

Al compartir las capas ocultas entre todos los _targets_, esta arquitectura fuerza a la red a aprender una representación interna común de las variables ambientales que sea útil para predecir todos los indicadores de biodiversidad simultáneamente, un enfoque de _multi-task learning_ que puede mejorar la generalización, si los _targets_ comparten estructura, pero que también puede perjudicar a un _target_ concreto si sus patrones difieren mucho de los del resto (efecto conocido como _negative transfer_).
...
La función de pérdida implementada para este caso combina MSE estándar con un término que penaliza que la matriz de correlación entre los 21 _targets_ predichos se aleje de la matriz de correlación entre los _targets_ reales del lote de datos de entrenamiento:

$ L(y, hat(y)) = "MSE"(y, hat(y)) + lambda_"corr" parallel "Corr"(y) - "Corr"(hat(y)) parallel_F $

donde:
- *$"Corr"(x)$* es la matriz de correlación de Pearson entre los 21 _targets_ calculada sobre el lote.
...
=== Comparación de los modelos empleados <comparativa-modelos>

En la #ref(<tab-3>) se muestra una comparativa rápida de los modelos descritos en las secciones anteriores.
...
La exactitud o el _accuracy_ mide la proporción de muestras cuya clase predicha coincide con la clase real:

$ "Acc" = 1/n sum_(i=1)^n bb(1)(hat(y)_i^"cls" = y_i^"cls") $

donde:
- *$bb(1)( · )$* es la función indicadora#sub([@scikitLearn]).
...
Aún así, la exactitud no distingue el tipo de error cometido, por lo tanto se suele recurrir al #link(<kappa-de-cohen>)[*Coeficiente de Kappa de Cohen*], explicado más adelante en el presente documento.

==== Precisión, _recall_ y F1 <F1>
...
$ "Precisión"_c = frac("TP"_c, "TP"_c + "FP"_c) quad quad "_Recall_"_c = frac("TP"_c, "TP"_c + "FN"_c) $

donde TPc, FPc y FNc son, respectivamente, los verdaderos positivos, los falsos positivos y los falsos negativos de la clase c#sub([@scikitLearn]). La precisión indica “de las muestras que el modelo etiquetó como c, cuántas lo eran realmente”, mientras que el _recall_ indica “de las muestras que realmente eran c, cuántos detectó el modelo”.
...
$ "F1"_c = 2 dot frac("Precisión"_c dot "_Recall_"_c, "Precisión"_c + "_Recall_"_c) $
...
Normalizada por filas, la matriz de confusión se puede leer directamente como una tasa de aciertos y de tipos de error por clase, lo que facilita la comparación entre _targets_ con distinto número de muestras y entre modelos con distinta exactitud global.
...
La arquitectura explicada previamente se compone de cuatro capas, las cuales transforman los datos brutos en uno o varios modelos predictivos para su futura evaluación y comprobación. Dicha estructura de capas se puede ver en el diagrama de la #ref(<fig-2>).
...
+ *Procesado de datos de ficheros:* En este subproceso nos centramos en los datos que se tienen ya tanto del proyecto SOB4ES, como de otros _datasets_ ya existentes, tanto a nivel UVIGO, como a nivel europeo.
  ...
  + *Datasets:*
      - *model_ready.csv:* _Dataset_ optimizado ya para su uso e introducción en los diferentes modelos que se desarrollen a futuro.
      - *clean.csv:* _Dataset_ intermedio con todos los datos limpios, empleado en las siguientes fases del procesamiento de datos como referencia y para la armonización de sus datos con los datos obtenidos por consultas remotas a bases de datos en la red.
...
- *sklearn/scikit-learn#sub([@scikitLearn]):* Librería principalmente creada para _Machine Learning_ y para análisis de datos. Dentro de la propia librería hacemos uso de las funciones/submódulos que se muestran en la #ref(<tab-5>):
  ...
  #figure(
    align(center)[
      ...
      table.cell(align: center)[*multioutput.\ RegressorChain*], 
        table.cell(align: left)[Permite envolver un modelo base (en nuestro caso _Random Forest_) para, durante el entrenamiento, replicar el modelo base una vez por cada _target_. De esta forma se pueden encadenar las predicciones de los _targets_ anteriores como entrada adicional para poder predecir los siguientes valores.],
      ...
      table.cell(align: center)[*model_selection.\ RepeatKFold*], 
        table.cell(align: left)[Permite aplicar la estrategia de partición en pliegues para la validación, obteniendo así lo siguiente:\
          *_$ x "pliegues" times y "repeticiones" = "xy" "evaluaciones" $_*
      ],
...
- *PyTorch#sub([@pytorchDocs]):* Librería que se emplea principalmente para actividades de ML, y para la creación de redes neuronales. Actualmente es una de las más empleadas dentro del entorno académico e investigador.
  ...
  De esta librería destacamos los siguientes elementos en la #ref(<tab-6>):
...
+ *Fase de validación individual:* Durante el desarrollo de los modelos, en cada _notebook_ se han integrado varias celdas que tienen como objetivo realizar la validación cruzada para cada modelo antes de generar todos los modelos resultantes.
...
+ *Fase de validación en conjunto de los modelos generados:* Para el correcto desarrollo de esta fase se van a crear dos _notebooks_ más que se van a encargar de realizar la evaluación y validación de los modelos con dos puntos de vista distintos, los cuales son:
  ...
  - *Punto de vista numérico:* En este _notebook_ nos centramos en cargar todos los modelos desarrollados y evaluar el rendimiento de cada uno de los modelos contra el _dataset_ creado para las pruebas.\
    ...
    - *RMSE.*
    - *MAE.*
    A partir de la ejecución del _notebook_ se podrán ver los resultados de las pruebas tanto en las celdas de salida del _notebook_, como en los archivos de resultado que se exportan en la última fase de ejecución. Estos archivos contienen los resultados de la comparación entre modelos y el $R^2$ por cada _target_ de todos los modelos probados.

  - *Punto de vista discreto:* Este _notebook_ sigue una metodología similar para la realización de pruebas a la del _notebook_ anterior, pero en este caso se realiza una clasificación para poder ver los resultados, no tanto como números, sino como aciertos y fallos.
...
- *scikit-learn:* Véase definición anterior. Para esta capa hacemos uso de los siguientes submódulos de scikit-learn, los cuales se pueden ver en la #ref(<tab-7>):
  ...
  #figure(
    align(center)[
      ...
      table.cell(align: center)[*base.BaseEstimator*], 
        table.cell(align: left, rowspan: 2)[Clases empleadas para envolver un modelo creado con PyTorch en una interfaz compatible con scikit-learn. Es necesario para poder hacer uso de permutation_importance sobre redes neuronales, ya que por defecto son incompatibles.],
...
En esta sección se justifican las herramientas, servicios y programas de terceros empleados a lo largo de este trabajo. Parte de estos elementos se encuentran mencionados en #link(<arquitectura>)[*Arquitectura general*], vista ya anteriormente, donde se describió cómo se integran dentro del flujo de datos.

Aquí se explica, para cada uno, por qué se ha elegido frente a las alternativas disponibles.

=== Lenguaje de programación, control de versiones y bibliotecas <lenguaje-de-programacion-control-de-versiones-y-librerias>
