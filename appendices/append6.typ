== Distribución de ramas y _notebooks_ (Anexo VI) <dist-ramas-y-notebooks>
...
#colbreak()
  ...
  Además de la #ref(<tab-44>), en la #ref(<fig-22>) se puede ver de una forma más gráfica la distribución de las ramas de todo el repositorio.
...
#figure( 
  align(center)[ 
    #table( 
      ...
      fill: (col, row) => if row == 0 or col == 0 { rgb("d6e3da") }, 
    table.header( 
      table.cell(align: center)[*_Notebook_*], 
...
Todos estos _notebooks_ comparten una misma estructura interna (carga de datos y configuración, búsqueda de hiperparámetros, validación cruzada del modelo final, entrenamiento final, evaluación preliminar y persistencia del modelo en `.pkl` mediante `joblib`), con el objetivo de mantener una experiencia de desarrollo homogénea entre modelos y facilitar así tanto la comparación entre ellos como la incorporación de nuevos modelos en un futuro. Sobre esta estructura común, cada _notebook_ incorpora las particularidades propias de su modelo, como las ya mencionadas en la tabla anterior.

==== Comparación de resultados

Una vez entrenados todos los modelos anteriores, los _notebooks_ que se muestran en la #ref(<tab-46>) se encargan de cargarlos (o los datos exportados por los mismos) y compararlos entre sí, cada uno desde uno de los dos puntos de vista descritos en la #link(<capa-evaluacion>)[*Capa de evaluación de modelos*]. Estos _notebooks_ se pueden ver con más detalle dentro del #link(<elementos-auxiliares>)[*Anexo IV*].
...
#figure( align(center)[ 
  ...
  table.cell()[*comparador_modelos*], 
    table.cell()[Carga de todos los modelos entrenados (`.pkl`) y generación de métricas comparativas desde el punto de vista numérico, las cuales se han descrito ya en la #link(<capa-evaluacion>)[*Capa de evaluación de modelos*].], 
  table.cell()[*comparador_clasificación*], 
    table.cell()[Carga de todos los modelos entrenados (`.pkl`) y generación de métricas comparativas desde el punto de vista ordinal (clasificación), las cuales se han descrito ya en la #link(<capa-evaluacion>)[*Capa de evaluación de modelos*].], 
  ...
  table.cell()[*comparador_barrido_rs*], 
    table.cell()[Carga de todos los modelos entrenados (`.pkl`) y generación de métricas comparativas desde el punto de vista ordinal (clasificación) y numérico, las cuales se han descrito ya en la #link(<capa-evaluacion>)[*Capa de evaluación de modelos*]. Forma parte de la #link(<prueba-3>)[*Prueba de barrido de random_state*]],
  )], 
  caption: [_Notebooks_ de comparación de resultados.], 
  kind: table 
)<tab-46>
...
Vinculados a las ramas *`model-prep-mixin`* y *`model-prep-mixin-1`* (véase la #ref(<tab-44>)), los dos _notebooks_ que se pueden ver en la #ref(<tab-47>) exploran si combinar las predicciones de varios modelos base mejora el resultado obtenido por cualquiera de ellos por separado. Estos _notebooks_ se pueden ver con más detalle en el #link(<elementos-auxiliares>)[*Anexo IV*].
...
#figure( 
  align(center)[ 
    ...
    table.cell()[*prueba_ensamblado*], 
      ...
      caption: [_Notebooks_ de pruebas de ensamblado.], 
      kind: table, 
    )<tab-47>
