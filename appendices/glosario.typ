...
#figure(
  ...
  #table(
    ...
    [*Kappa de Cohen*], [Métrica de concordancia entre la predicción y el valor real que corrige el nivel de acierto esperable por azar. Más robusta y fiable que el _Accuracy_, especialmente cuando las clases están desbalanceadas.],
    ...
    [*_Raster_*],[Formato de datos geoespaciales que representa el espacio como una malla regular de celdas (píxeles), cada una con un valor asociado (p. ej. elevación, temperatura, NDVI), a diferencia del formato vectorial (puntos, líneas, polígonos). Las variables descargadas de fuentes remotas como GEE o Copernicus DEM se distribuyen originalmente en este formato.],
