...
+ *Evaluación del modelo:* Validación con los correspondientes parámetros de evaluación del rendimiento estadístico frente a datos de control ciegos (nunca empleados en entrenamiento ni en otras pruebas anteriores a la evaluación). 
...
+ *Procesamiento de la Información:* 
  El tratamiento masivo de estos datos ambientales se realizará utilizando diversas bibliotecas de Python. Se utilizarán `pandas`, `numpy` y otras librerías para la limpieza, manipulación eficiente de variables tabulares y escritura de archivos CSV/XLSX. Para el manejo de datos geoespaciales y archivos multidimensionales provistos por Copernicus y bases europeas, se hará uso herramientas específicas como `rasterio`, `xarray` y `dbfread`. Dichas librerías se mencionan con más detalle en #link(<arquitectura>)[*Arquitectura empleada*].
